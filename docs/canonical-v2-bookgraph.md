...
- 抽取 TOC entries 的标题、目录编号、层级和目录印刷页码；目录印刷页码是 TOC evidence，
  但不用来定位 PDF 物理页。
- 用 ObservedDocument 的 `title_text` / text observations 定位标题出现的 PDF 物理页。

每个 `toc_entry` 的 shadow contract 至少包含：

- `raw_title`: TOC OCR 中去掉印刷页码后的原始标题文本，例如 `I 日本：从战国时代到世界强权`。
- `title`: 不含目录编号的标题正文，例如 `日本：从战国时代到世界强权`。
- `display_title`: 规范化目录编号后的展示标题，例如 `1 日本：从战国时代到世界强权`。
- `raw_label`: TOC OCR 中识别出的原始目录编号，例如 `I`。
- `label`: 规范化后的目录编号，例如 `1`。
- `printed_start_page`: 目录里印刷的起始页码，保留为字符串；它不是 PDF 物理页。
- `candidate_start_pages`: 规则层定位出的 PDF 物理起始页候选。
- `selected_start_page`: 从候选中基于 TOC 顺序和局部证据选出的 PDF 物理起始页。
- `level`: TOC 层级。一级部分/章节为 `1`，其下编号小节为 `2`。
- `parent_entry_index`: 最近上级 TOC entry 的 `entry_index`，没有上级时为 `null`。

可选 LLM 层只基于 TOC entry `display_title` / `title` / `label` / `level` 输出
`front_matter` / `body` / `back_matter` 的 entry role 和 entry index 边界；它不能输出或决定
PDF 物理页码。物理页只来自规则层 title location evidence。这个约束是为了避免“LLM 猜页码”污染
pipeline，同时让后续 BookGraph 构造可以先拥有书的宏观骨架，再生成 paragraph / display_block / note 等节点。

LLM verifier 的输入应由 audit 信号触发，而不是逐页调用。Phase 4 当前会为 `raw_label != label`
的 TOC 编号修正输出 `label_ocr_corrected` issue，例如把 OCR 的 `I` 规范化为 `1`；这类 issue
会带 `llm_review_recommended: true`，后续可作为小批量 LLM 校验输入。
