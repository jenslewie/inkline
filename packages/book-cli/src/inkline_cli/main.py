...
from book_canonical.io import read_canonical, read_jsonl, write_canonical, write_jsonl
from book_epub import export_epub
from book_ingest import import_epub
from book_rag.chunks import export_chunks
from book_rag.embeddings import OpenAIEmbeddingClient
from book_rag.faiss_index import build_faiss_index, load_faiss_index
from book_rag.search import dense_search, load_docstore
from book_rag.types import dataclass_to_dict


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="inkline", description="Book ingestion, export, and RAG pipeline.")
    ...
    ingest_pdf.add_argument("--engine", default="mineru", choices=["mineru"])
    ingest_pdf.add_argument("--backend", default="vlm-auto-engine", help="MinerU backend. Defaults to vlm-auto-engine.")
    ingest_pdf.add_argument("--method", default="auto", help="MinerU parse method. Defaults to auto.")
...
def _ingest_pdf(args: argparse.Namespace) -> int:
    from book_mineru.bridge import ingest_pdf_with_mineru

    document = ingest_pdf_with_mineru(args.input, engine=args.engine, backend=args.backend, method=args.method, output=args.output)
