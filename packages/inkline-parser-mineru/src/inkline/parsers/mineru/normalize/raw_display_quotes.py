...
def should_start_display_quote(
    ...
    strongly_indented = bool(b.bbox and b.x0 >= layout.body_left + 55 and b.width <= layout.body_width * 0.95)
    short_intro_context = bool(prev_text.strip()) and len(normalize_ws(prev_text)) <= 90 and prev_text.rstrip().endswith(("。", "！", "？", ".", "）", ")"))
    if strongly_indented and len(t) >= 100 and short_intro_context and next_raw_type == "page_footnote":
        return True
    if is_page_bottom_set_off_before_footnotes(blocks, i, layout, text_style):
        return True
    if i + 2 < len(blocks):
        tri = blocks[i:i+3]
        if all(x.raw_type == "paragraph" and len(block_text(x)) <= 24 and is_short_or_indented(x, layout) for x in tri):
            return True
    if RawSetOffDisplayRunDetector(layout).collect(blocks, i)[0]:
        return True
    return False
...
def is_page_bottom_body_tail_before_footnotes(blocks: List[RawBlock], i: int, layout: LayoutStats) -> bool:
    b = blocks[i]
    if not b.bbox:
        return False
    t = block_text(b)
...
def is_page_bottom_set_off_before_footnotes(
    ...
    if not following_same_page or not all(x.raw_type == "page_footnote" for x in following_same_page):
        return False
    prev = None
    ...
    if gap < max(12.0, page_height * 0.015):
        return False
    if prev.width < layout.body_width * 0.84 or prev.x0 > layout.body_left + max(55.0, layout.body_width * 0.07):
        return False
    global_indent = b.x0 - layout.body_left
    local_indent = b.x0 - prev.x0
    global_set_off = global_indent >= max(58.0, layout.body_width * 0.07) and b.width <= layout.body_width * 0.95
    local_set_off = local_indent >= max(36.0, layout.body_width * 0.045) and b.width <= prev.width - max(24.0, layout.body_width * 0.03)
...
def collect_display_quote(
    ...
    while j < len(blocks):
        ...
        if not group:
            ...
            continue
        prev = group[-1]
        if ends_with_terminal_punctuation(block_text(prev)) and is_page_bottom_body_tail_before_footnotes(blocks, j, layout):
            break
        if ATTR_RE.match(t):
            group.append(b)
            j += 1
        ...
        if is_right_aligned_short(b, layout):
            ...
            j += 1
            continue
        if _body_layout_resumes_after_completed_display(group[0], prev, b, blocks, layout, text_style):
            break
        ...
        if is_left_shifted_intro_before_display_lane(blocks, j, layout, reference_x0=group[0].x0 if group[0].bbox else None):
            break
...
def _body_layout_resumes_after_completed_display(
    ...
    if body_size is not None and candidate_size >= body_size * 0.94:
        return True
    if first_size is not None and candidate_size >= first_size * 0.95:
        return True
    return False
...
def _short_line_before_body_layout_resumes(
    ...
    if body_size is not None and candidate_size >= body_size * 0.98:
        return True
    if first_size is not None and candidate_size >= first_size * 1.04:
        return True
    return False
...
def _next_same_page_body_aligned(blocks: Sequence[RawBlock], candidate: RawBlock, layout: LayoutStats) -> bool:
    try:
    ...
    for nxt in blocks[start:]:
        ...
        if nxt.raw_type != "paragraph" or not nxt.bbox or not block_text(nxt):
            return False
        return abs(nxt.x0 - candidate.x0) <= max(12.0, layout.body_width * 0.025) and nxt.width >= layout.body_width * 0.86
    return False
