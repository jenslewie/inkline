"""Build a Phase 4A PageReview directly from MinerU raw artifacts."""
...
def parse_args() -> argparse.Namespace:
    """Parse raw MinerU inputs and the one required PageReview output path."""

    parser = argparse.ArgumentParser(
        description="Build a Phase 4A PageReview directly from MinerU raw artifacts."
    )
    ...
    parser.add_argument("--source-pdf", help="Original PDF used to render LLM page-review images.")
    parser.add_argument(
    ...
    parser.add_argument("--output", required=True, help="PageReview JSON output path.")
    parser.add_argument(
        "--skeleton-llm",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use the local multimodal LLM to build the TOC-driven BookSkeleton.",
    )
    parser.add_argument(
        "--llm",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use the local multimodal LLM for selected visual page review.",
    )
...
def main() -> None:
    """Write only a PageReview artifact and its resumable LLM checkpoint."""

    args = parse_args()
    args.source_pdf = resolve_source_pdf_path(
        args.source_pdf, allow_missing=args.allow_missing_pdf_text
    )
    if (args.llm or args.skeleton_llm) and not args.source_pdf:
        raise ValueError("--llm or --skeleton-llm requires a readable --source-pdf.")
    ...
    skeleton = build_book_skeleton_shadow(
        observed,
        use_llm=args.skeleton_llm,
        source_pdf=args.source_pdf,
        image_output_dir=output_path.parent / "book_skeleton_toc_llm_pages",
    ...
    review = build_page_review_shadow(
        ...
        llm_api_url=args.llm_api_url,
        llm_timeout_seconds=args.llm_timeout_seconds,
        checkpoint_path=output_path.parent / "page_review.checkpoint.json",
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(review, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {output_path} with {len(review['candidate_pages'])} review candidate page(s)")
