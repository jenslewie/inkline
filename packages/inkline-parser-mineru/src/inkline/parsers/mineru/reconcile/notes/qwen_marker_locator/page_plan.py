...
def _problem_page_plan(blocks: List[CanonicalBlock]) -> _ProblemPagePlan:
    ...
    for page, footnotes in footnotes_by_page.items():
        markers = [
            leading_note_marker(str(block.get("text") or ""), include_superscript=True)
            for block in footnotes
        ]
        if any(marker is None for marker in markers) or any(
            _needs_footnote_definition_review(block) for block in footnotes
        ):
        ...
        body_candidate_block_ids.update(
            _anchored_body_candidate_block_ids(blocks, page, markers, refs_by_page.get(page, []))
        )
...
def _anchored_body_candidate_block_ids(
    ...
    defs = {
        marker_int for marker in footnote_markers if (marker_int := _marker_int(marker)) is not None
    }
    ...
    for missing in sorted(defs - ref_markers):
        left = max(
            (marker for marker in defs if marker < missing and marker in ref_markers), default=None
        )
        right = min(
            (marker for marker in defs if marker > missing and marker in ref_markers), default=None
        )
...
def _endnote_body_candidate_block_ids(blocks: List[CanonicalBlock]) -> Set[int]:
    ...
    for scope_key, defs in _chapter_endnote_defs_by_scope(blocks, context).items():
        ...
        candidates.update(
            _anchored_scope_candidate_block_ids(blocks, context, refs, defs, scope_key=scope_key)
        )
    book_defs = _book_endnote_defs(blocks, context)
    if book_defs:
        refs = _body_ref_items_for_scope(blocks, context, None)
        candidates.update(
            _anchored_scope_candidate_block_ids(blocks, context, refs, book_defs, scope_key=None)
        )
    return candidates


def _chapter_endnote_defs_by_scope(
...
def _closest_anchor_span(
    refs: Sequence[tuple[int, CanonicalBlock, int]], left_anchor: int, right_anchor: int
...
def _fallback_page_body_candidate_block_ids(blocks: List[CanonicalBlock], page: int) -> Set[int]:
    return {id(block) for block in blocks if _is_body_ref_candidate_block(block, page)}
...
def _is_scope_body_ref_candidate_block(
    block: CanonicalBlock, context: _NoteContext, scope_key: Optional[str]
) -> bool:
    if block.get("type") not in BODY_TYPES:
        return False
    if scope_key is not None and context.scope_for(block) != scope_key:
