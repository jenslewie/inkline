"""Qwen-backed missing note reference recovery."""
...
from .resolver import _NoteContext, _PageFootnoteStrategy

__all__ = ["recover_missing_note_refs"]


def recover_missing_note_refs(blocks: List[Dict[str, Any]], _source_pdf: Any = None, *_args: Any, **_kwargs: Any) -> None:
    """Recover inline note refs only from Qwen visual marker evidence."""
    context = _NoteContext(blocks)
    page_defs = _collect_page_note_definition_markers(blocks, context)
    page_symbol_defs = _collect_page_symbol_definition_markers(blocks, context)
    if not page_defs and not page_symbol_defs:
        return
...
def _collect_page_note_definition_markers(blocks: List[Dict[str, Any]], context: _NoteContext) -> Dict[int, Set[int]]:
