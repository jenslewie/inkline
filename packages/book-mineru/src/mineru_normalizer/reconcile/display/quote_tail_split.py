"""Page-bottom quote overflow tail split. Splits a page-bottom display block when its final line is prose narrative rather than displayed text, converting the tail back to a paragraph and merging it with the next page's prose."""
...
from ..common import (
    ...
    _ends_with_terminal, _is_near_page_bottom, _is_near_page_top,
    _merge_block_pair, _note_ref_key, _page_coord_heights,
    _prev_text_non_float, _refresh_canonical_quote_attrs,
)


def reconcile_page_bottom_overflow_tail_from_quotes(blocks: List[Dict[str, Any]], layout: LayoutStats) -> None:
    ...
    while i + 1 < len(blocks):
        ...
        if "\n" not in text:
            i += 1
            continue
        quote_text, tail = text.rsplit("\n", 1)
        quote_text = quote_text.rstrip()
        tail = tail.lstrip()
        if not quote_text or not tail:
            i += 1
            continue
        if not _ends_with_terminal(quote_text) or _ends_with_terminal(tail):
            i += 1
            continue
        quote_runs, tail_runs = _split_inline_runs_at_last_newline((b.get("attrs") or {}).get("inline_runs"))
        quote_refs, tail_refs = _split_note_refs_by_runs((b.get("attrs") or {}).get("note_refs"), quote_runs, tail_runs)
        b["text"] = quote_text
        _refresh_canonical_quote_attrs(b, prev_text=_prev_text_non_float(blocks, i))
        attrs = b.setdefault("attrs", {})
        if quote_runs:
            attrs["inline_runs"] = quote_runs
...
def _split_inline_runs_at_last_newline(runs: Any) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for index, run in enumerate(runs):
        ...
        if index < split_run_index:
            ...
            if before:
                quote_runs.append({"type": "text", "text": before})
            if after:
                tail_runs.append({"type": "text", "text": after})
    return quote_runs, tail_runs
...
def _split_note_refs_by_runs(
    ...
    return _refs_for_runs(quote_runs, buckets), _refs_for_runs(tail_runs, buckets)
