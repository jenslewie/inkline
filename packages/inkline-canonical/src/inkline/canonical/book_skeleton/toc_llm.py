...
def book_skeleton_toc_llm_prompt(input_data: dict[str, Any]) -> str:
    return (
        ...
        "not '缩 写', '年 表', or '致 谢'.\n"
        "- Do not preserve typographic tracking between Chinese characters. Normalize '终 章', "
        "'后 记', and '注 释' to '终章', '后记', and '注释'. This does not remove the single "
        "separator after a structural prefix: keep '第一章 楼兰' and '第四部分 僵局'.\n"
        ...
        "- Establish that a visual line is a separate TOC entry before assigning its level or parent. "
        "An independent page locator, structural numbering, or clearly separate entry grouping can "
        "establish entry identity. Indentation determines hierarchy only after a line is established "
        "as a separate entry.\n"
        "- Once independent entry identity is established, visual indentation is decisive for "
        "hierarchy. Do not flatten an independently numbered or locator-bearing entry merely because "
        "it has its own label. For example, a visibly indented '专题1 文献专题 115' beneath a chapter "
        "is a separate level-2 child of that chapter, not a level-1 entry.\n"
        "- A visual line does not become a separate entry merely because it is indented. When an "
        "indented subtitle follows a numbered chapter line that already owns the page locator, has no "
        "independent page locator, and the image presents both lines as one chapter-title group, append "
        "the subtitle to the preceding display_title instead of emitting a child entry. Preserve the "
        "line boundary with '\\n'. For example, '第1章 城市的诞生 21' followed by the indented "
        "'王朝初建，750—775年' is one display_title: "
        "'第1章 城市的诞生\\n王朝初建，750—775年'.\n"
        "- Assign parent_entry_index only after both the parent and child have independent entry "
        "identity and the TOC visually indents the child under the earlier entry. Entries with the "
        "same left alignment must have the same level, even when adjacent entries look related as "
        "tables, lists, notes, sources, chronology, or other back matter.\n"
        ...
        "- Do not merge two independently identified TOC entries. A wrapped title or a subtitle "
        "continuation without its own locator is part of one entry even when it occupies a separate "
        "visual line.\n"
        ...
        "Before returning JSON, perform a final contract check: compact tracked Chinese words, "
        "preserve structural-prefix separators, retain every independently identified entry, merge "
        "only subtitle continuations without independent identity, and verify every non-level-1 "
        "entry points to the visually governing earlier parent.\n\n"
