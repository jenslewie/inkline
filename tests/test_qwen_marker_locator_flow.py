...
from inkline.parsers.mineru.normalizer.canonical.core import (
    _marker_locator_block_dpi,
    _marker_locator_page_dpi,
    _missing_or_unreliable_body_ref_pages,
)
from inkline.parsers.mineru.normalizer.reconcile.notes import qwen_marker_locator
from inkline.parsers.mineru.normalizer.reconcile.notes.qwen_evidence import (
    _collect_qwen_marker_evidence,
    _page_footnote_markers_by_page,
    _retry_missing_single_marker_body_refs,
)
from inkline.parsers.mineru.normalizer.reconcile.notes.qwen_api import _call_qwen_marker_locator
from inkline.parsers.mineru.normalizer.reconcile.notes.qwen_page_plan import _problem_page_plan
from inkline.parsers.mineru.normalizer.reconcile.notes.qwen_marker_locator import (
    QwenMarkerLocatorConfig,
    QwenMarkerPageEvidence,
    run_qwen_marker_locator_repairs,
)
from inkline.parsers.mineru.normalizer.reconcile.notes.qwen_types import _PROMPT_VERSION
...
def test_page_then_block_retries_missing_pages_with_block_dpi(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_page_plan._problem_page_plan", fake_problem_page_plan)
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_evidence._collect_qwen_marker_evidence", fake_collect)
...
def test_single_marker_retry_merges_missing_marker(monkeypatch, tmp_path: Path) -> None:
    ...
    # Patch at the definition module (monkeypatch principle)
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_api._call_qwen_marker_locator", fake_call)
    config = QwenMarkerLocatorConfig(
        source_pdf=tmp_path / "source.pdf",
        artifact_dir=tmp_path / "qwen",
    )
...
def test_complete_cache_hit_does_not_raise_unboundlocalerror(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_evidence._read_existing_evidence", MagicMock(return_value=fake_cache))
    # Patch render to avoid real PDF rendering
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_prompt._render_full_page", MagicMock())
    # Patch timing to avoid needing a real config with all fields
    monkeypatch.setattr("inkline.parsers.mineru.normalizer.reconcile.notes.qwen_evidence._write_timing_event", MagicMock())
